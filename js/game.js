...
const preloadedCoffee = new Image(); preloadedCoffee.src = 'assets/coffee.png'; 
const preloadedCopilot = new Image(); preloadedCopilot.src = 'assets/copilot.png';
const preloadedCheer = new Image(); preloadedCheer.src = 'assets/female_cheer2.png';
...
class GameManager {   
    constructor() {     
        ...
        this.portals = [       
            { id: 'word', img: this.apps.word, originalImg: this.apps.word, x: 250, y: 350 },       
            { id: 'pwp', img: this.apps.pwp, originalImg: this.apps.pwp, x: 800, y: 550 },       
    ...
    takeDamage() {       
        if (this.invulnerable || this.isCelebrating) return;       
    ...
    selectCategory(category, targetElement) {     
        if (this.level < 3 || this.isCelebrating || this.activePortal) return;      
    ...
    cancelPortal() {
        if (this.activePortal) {
            this.activePortal.img = this.activePortal.originalImg; 
            this.activePortal = null;
            player.frozen = false;
    ...
    drawPortals(ctx) {     
        ...
        this.portals.forEach((p, index) => {       
            ...
            if (p.img.complete && p.img.naturalWidth > 0) {         
                const maxImgSize = 44;
                const scale = Math.min(maxImgSize / p.img.naturalWidth, maxImgSize / p.img.naturalHeight);
                const drawW = p.img.naturalWidth * scale;
                const drawH = p.img.naturalHeight * scale;
                ctx.drawImage(p.img, centerX - drawW / 2, centerY - drawH / 2, drawW, drawH);       
            }              
...
window.addEventListener('keydown', e => {   
    if (gameManager.isCelebrating) return; 

    if (gameManager.activePortal) {
        if ((e.ctrlKey || e.metaKey) && (e.code === 'KeyV' || e.key === 'v')) {
            e.preventDefault();
            gameManager.handleDrop(player.carriedPrompt, gameManager.activePortal.id);
            return;
        }
        if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'KeyW', 'KeyA', 'KeyS', 'KeyD'].includes(e.code)) {
            e.preventDefault();
            gameManager.cancelPortal();
            return;
        }
        return; 
    }
...
function gameLoop() {   
    ...
    if (!gameManager.isCelebrating && !gameManager.activePortal) {
        ...
        if (gameManager.levelTime - gameManager.lastEmailTime >= emailInterval) {       
            ...
            gameManager.emails.push(new EmailProjectile(spawnX, spawnY, facingLeft));   
        }
    }
    ...
    if (player.carriedPrompt && !gameManager.activePortal && player.portalCooldown === 0) {     
        gameManager.portals.forEach(portal => {       
            if (player.x < portal.hitX + portal.hitW &&         
    ...
    if (gameManager.activePortal && !gameManager.isCelebrating) {
        player.x = gameManager.activePortal.hitX + gameManager.activePortal.hitW / 2 - player.w / 2;
        player.y = gameManager.activePortal.hitY - player.h + 20;
    }

    if (gameManager.invulnerable && !gameManager.isCelebrating) {       
